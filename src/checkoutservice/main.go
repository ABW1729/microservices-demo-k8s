...
type checkoutService struct {
    ...
    db *sql.DB // ✅ new field for database
}
...
func main() {
  ...
  dbHost := os.Getenv("DB_HOST")
	...
	svc.db = db
	log.Infof("connected to Postgres database at %s:%s", dbHost, dbPort)
...
func (cs *checkoutService) PlaceOrder(ctx context.Context, req *pb.PlaceOrderRequest) (*pb.PlaceOrderResponse, error) {
	...
	_, err = cs.db.ExecContext(ctx,
		  `INSERT INTO orders (order_id, user_id, total_amount, currency, transaction_id, shipping_id, created_at)
		   VALUES ($1, $2, $3, $4, $5, $6, NOW())`,
		  orderID.String(),
		  req.UserId,
		  fmt.Sprintf("%d.%d", total.Units, total.Nanos),
		  req.UserCurrency,
		  txID,
		  shippingTrackingID,
	)
	if err != nil {
		  log.Warnf("failed to save order in DB: %v", err)
	} else {
		  log.Infof("order %s saved in database", orderID.String())
	}
